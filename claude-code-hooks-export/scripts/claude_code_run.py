...
DEFAULT_CLAUDE = os.environ.get("CLAUDE_CODE_BIN", "/Users/defi/.local/bin/claude")
...
def run_with_pty(cmd: list[str], cwd: str | None, env: dict[str, str] | None = None) -> int:
    ...
    # Check if script supports -c (Linux) or needs different approach (macOS)
    # macOS: script -q /dev/null cmd args...
    # Linux: script -q -c "cmd args..." /dev/null

    # Try Linux style first
    cmd_str = " ".join(shlex.quote(c) for c in cmd)
    proc = subprocess.run([script_bin, "-q", "-c", cmd_str, "/dev/null"],
                         cwd=cwd, text=True, env=env, capture_output=True)

    # If failed with illegal option, try macOS style
    if proc.returncode != 0 and "illegal option" in proc.stderr:
        proc = subprocess.run([script_bin, "-q", "/dev/null"] + cmd,
...
def run_interactive_tmux(args: argparse.Namespace) -> int:
    ...
    socket_dir = args.tmux_socket_dir or os.environ.get("CLAUDE_CODE_TMUX_SOCKET_DIR") or f"{os.environ.get('TMPDIR', '/tmp')}/claude-code-tmux-sockets"
...
def main() -> int:
    ...
    ap.add_argument("--tmux-socket-dir", default=None, help="tmux socket dir (defaults to $CLAUDE_CODE_TMUX_SOCKET_DIR or /tmp)")
